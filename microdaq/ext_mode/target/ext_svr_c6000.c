 ...
 *
 * File: ext_svr.c     $Revision: 1.1.6.1 $
 *
 * Abstract:
 ...
 */
PUBLIC const char_T *ExtInitUD(void)
{
 ...
 *  mode. 
 */
PUBLIC boolean_T rt_ExtModeInit()
{
    int i;
    boolean_T error = EXT_NO_ERROR;
                             ...
                             boolean_T      *stopReq)
{
    ...
    if (error != EXT_NO_ERROR) {
	#if defined(VERBOSE) && defined(ENABLE_PRINTFS)
 ...
 * Abstract:
 *  Call rt_PktServerWork forever.   Used only for RTOS (e.g., Tornado/C6000
